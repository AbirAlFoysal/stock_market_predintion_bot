...
csv_directory = r'D:\AbleAid\StockPradiction\DB_csv\unadjusted_amarstock'  # Replace with your directory path

# Directory to save the models
model_directory = r'D:\AbleAid\StockPradiction\models'  # Replace with your model directory path
...
for filename in tqdm(os.listdir(csv_directory), desc="Processing CSV files"):
    if filename.endswith('.csv'):
        ...
        if len(df) > 2:
            ...
            # Predict using the trained model (fitting it to itself)
            y_pred = gb.predict(X)
            ...
            inquiry_df.loc[inquiry_df['share'] == filename.replace(".csv", ""), 'model'] = last_date
